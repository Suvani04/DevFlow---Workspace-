    ...
    <div className="min-h-screen bg-[#0a0b0f] flex">
      ...
      <div className="hidden lg:flex w-1/2 bg-[#111318] border-r border-[#1e2130] flex-col justify-between p-12">
        ...
        <div>
          <h2 className="text-[#e8eaf0] text-2xl font-extrabold mb-3">
            Your team's workspace awaits
          </h2>
          <p className="text-[#6b7280] text-sm leading-relaxed mb-8">
            Manage projects, chat with teammates, and let AI handle the repetitive stuff.
          </p>

          {/* Mini Tasks */}
          <div className="flex flex-col gap-2">
            <p className="text-[#6b7280] text-xs font-semibold uppercase tracking-widest font-mono mb-1">
              // Active Sprint
            </p>
            {[
              { color: '#f87171', text: 'Build auth middleware', badge: 'AI reviewed' },
              { color: '#fbbf24', text: 'Design kanban board', badge: null },
              { color: '#4ade80', text: 'Setup MongoDB Atlas', badge: 'Done ✓' },
              { color: '#a5b4fc', text: 'Integrate Claude API', badge: 'AI gen' },
            ].map((task, i) => (
              <div key={i} className="bg-[#16181f] border border-[#1e2130] rounded-lg px-3 py-2.5 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: task.color }}></div>
                <span className="text-[#e8eaf0] text-xs font-medium flex-1">{task.text}</span>
                {task.badge && (
                  <span className="text-[#a5b4fc] text-[10px] font-mono bg-[#5b6af0]/10 px-2 py-0.5 rounded-full">
                    {task.badge}
                  </span>
                )}
              </div>
        ...
        <div>
          <blockquote className="text-[#6b7280] text-sm italic leading-relaxed">
            "DevFlow cut our standup time in half. The AI writes everyone's update automatically."
          </blockquote>
          <cite className="text-[#5b6af0] text-xs font-mono not-italic block mt-2">
            — Rahul S., Senior Engineer @ Razorpay
          </cite>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-[#e8eaf0] text-2xl font-extrabold mb-1">Welcome back</h1>
          <p className="text-[#6b7280] text-sm mb-7">Sign in to your workspace</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-[#e8eaf0] mb-1.5 flex justify-between">
                Password
                <a href="#" className="text-[#5b6af0] font-normal hover:underline">
                  Forgot?
                </a>
              </label>
              <input
